...
public:
	...
	if (!checkSub(board)) return false;
	return true;     
    }
    ...
    bool checkCol(vector<vector<char>>& board) {
        for (int i = 0; i < 9; i++) {
			...
			if (board[j][i] == '.') continue;
                        if (visited[board[j][i]-'0'-1]) return false;
                        visited[board[j][i]-'0'-1] = 1;
                }
        }
        return true;
    }
    bool checkSub(vector<vector<char>>& board) {
		...
		for (int j = 0; j < 9; j += 3) {
			...
			for (int k = 0; k < 3; k++) {
				for (int t = 0; t < 3; t++) {
					if (board[i+k][j+t] == '.') continue;
					if (visited[board[i+k][j+t]-'0'-1]) return false;
					visited[board[i+k][j+t]-'0'-1] = 1;
				}
			}
		}
	}
